...
output<-foreach(r = 1:nrow(fact))%dopar%{
  ...
  if(fact$var[r] == "high"){  sigma<-runif((Aspecies+Plantspecies), 0.02,0.09) 
  }else {sigma <- runif((Aspecies+Plantspecies), 0.002,0.009)
  ...
  params<-list(matrix=g,bw=bw,aw=aw,w=w,Amatrix=Amatrix,Pmatrix=Pmatrix,web=fact$web[r],
               gi=gi,ki=ki,Temp=Temp, sigma=sigma,A=Aspecies,P=Plantspecies,degree=degree,
               var=fact$var[r],h2=fact$h2[r],
...
save(output, file ="cluster_warming_1.RData")
